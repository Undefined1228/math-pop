...
- [ ] `AppModeToggle` — 인쇄/테스트 전환 토글 (기존 "새로 생성" 버튼 자리)
- [ ] `StageDropdown` — 단계 1~11 선택, `label` 전체 설명 표시 (`1단계 · 초1-1 · 한 자리 덧뺄셈`)
- [ ] `TestResultModal` — 단계명, 합격 여부, 정답 수, 정답률, 소요 시간 표시. 버튼: `다시 시도` / `틀린 문제만 다시 풀기` / `단계 변경` / `인쇄 모드로`
...
- [ ] 단계 선택 → `{ range, ops, mode, count, durationSec }` 자동 세팅 확인
- [ ] "테스트 시작" → 문제 생성 + 타이머 자동 시작
- [ ] 타이머 종료 → 자동 채점 + 결과 모달 표시
- [ ] 결과 모달: "다시 시도" → 같은 단계 재시작
- [ ] 결과 모달: "틀린 문제만 다시 풀기" → 비례 타이머로 재시작
- [ ] 결과 모달: "단계 변경" → 모달 닫고 단계 드롭다운 포커스
- [ ] 결과 모달: "인쇄 모드로" → appMode 전환
- [ ] 테스트 모드에서 문제지·답안 인쇄 동작 확인
