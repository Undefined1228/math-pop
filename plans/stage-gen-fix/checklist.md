...
- [ ] 커스텀 모드(print)에서 3d 범위 선택 시 곱셈·나눗셈 선택 가능한지 확인

## Phase 3 — stage.ts

- [ ] 8단계: range `'mix'`, ops `['add', 'sub']`, 레이블 수정
- [ ] 9단계: range `'triple'`, ops `['add', 'sub']`, 레이블 수정
- [ ] 10단계: range `'3d'`, ops `['mul']`, 레이블 수정
- [ ] 11단계: range `'3d'`, ops `['div']`, 레이블 수정
- [ ] 8단계 문제가 세·네자리 혼합 덧뺄셈으로 생성되는지 확인
- [ ] 9단계 문제가 세 수 혼합계산으로 생성되는지 확인
- [ ] 10단계 문제가 세자리×두자리로 생성되는지 확인
- [ ] 11단계 문제가 세자리÷두자리로 생성되는지 확인
